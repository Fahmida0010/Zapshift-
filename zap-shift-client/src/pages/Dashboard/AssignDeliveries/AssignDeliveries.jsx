...
const axiosSecure = useAxiosSecure();
 ...
 .get(`/parcels/rider?riderEmail=
${user.email}&deliveryStatus=driver_assigned`)
        return res.data; 
    }
    })
...
let message = `Parcel status is updated with
 ${status.split('_').join('')}`
...
.then(res => {
  ...
  return (
    <div>
   <h2 className="text-4xl">Parcels 
    pending Pickup :{parcels.length}</h2>
 ...
 <button className ='btn btn-warning text-black ms-2'>
    Reject</button> 
        ...
        <td>
   <button 
 onClick = { () => handleDeliveryStatusUpdate(parcel,
  'parcel_picked_up')}
 className ='btn btn-primary text-black'>
