...
const regionDistricts = serviceCenters.filter(c=>
     c.region === region);
    const destricts = regionDistricts.map(d=>
         d.district);
  return destricts ;
}
...
if (isDocument) {
  ...
  if (parcelWeight < 3) {
    ...
    let extraCharge = isSameDistrict ? extraWeight * 40 : extraWeight * 40;

    if (extraWeight > 0) {
      extraCharge = extraWeight * 40 + 40; 
    }
...
Swal.fire({
 ...
 Swal.fire({
  ...
  return (
    <div>
<h2 className="text-5xl fonr-bold">Send A Parcel</h2>
...
<form onSubmit={handleSubmit(handleSendParcel)}
 ...
 <div claaName='grid grid-cols-1 md:grid-cols-2 gap-12
  my-8'>
 ...
 <div className='grid grid-cols-1 md:grid-cols-2  gap-12'>
  ...
  <select {...register('senderRegion')} defaultValue="Pick a region" className="select">
    <option disabled={true}>Pick a Region</option>
   {
    ...
    {/* sender region */}
          <fieldset className="fieldset">
  <legend className="fieldset-legend">Sender Disticts</legend>
  <select {...register('senderDistrict')} defaultValue="Pick a District" className="select">
    <option disabled={true}>Pick a Distict</option>
   {
  ...
  <select {...register('receiverRegion')} defaultValue="Pick a region" className="select">
    <option disabled={true}>Pick a Region</option>
   {
    regions.map((r, i )=> <option key={i} value={r}>{r}</option>)
            ...
            {/* Receiver region */}
          <fieldset className="fieldset">
  <legend className="fieldset-legend ">Receiver Disticts</legend>
